...
Очень важно в процессе отслеживать, что ветка появилась и на какой из веток находишься в данный момент, с помощью команды просмотра существующих веток.
Ветка  (master) - корневая ветка.
...
**Важно** - находиться на ветке (master). В случае возникновения конфликата между версиями, появиться подсказка от Git, где можно выбрать форму слияния - какие изменения сохранить в финальной версии.

### Удаление веток

Для удаление отработанных веток, испльзуем команду:

    git branch -d <имя ветки которую удаляем>

**Важно** - находиться на ветке (master). 
